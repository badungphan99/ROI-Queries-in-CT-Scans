...
abdominal_path = [f for f in os.listdir(chest_dataset) if os.path.isdir(os.path.join(chest_dataset, f))]
for app in abdominal_path:
    ...
    abdominal_pr = os.path.join(abdominal_dataset, app, "masks", "Prostate")
    ...
    list_pr = [f for f in os.listdir(abdominal_pr) if os.path.isfile(os.path.join(abdominal_pr, f))]
    ...
    pr = [u[:len(u) - 4] for u in list_pr]
    rt = [u[:len(u) - 4] for u in list_rt]

    start = min(int(bl[0]), int(lf[0]), int(rf[0]), int(pr[0]), int(rt[0]))
    end = max(int(bl[len(bl)-1]), int(lf[len(lf)-1]), int(rf[len(rf)-1]), int(pr[len(pr)-1]), int(rt[len(rt)-1]))

    for i in range(start, start+5):
        path_img = os.path.join(abdominal_images, str(i).zfill(3) + ".png")
        img_data = cv2.imread(path_img, 0)
    ...
    ran_img = np.random.randint(low=start + 5, high=end - 6, size=5)
    for r in ran_img:
        path_img = os.path.join(abdominal_images, str(r).zfill(3) + ".png")
        img_data = cv2.imread(path_img, 0)
        imgwrite = os.path.join(label_0, datetime.datetime.now().strftime("%d-%m-%Y:%H:%M:%S:%f") + ".png")
        cv2.imwrite(imgwrite, img_data)