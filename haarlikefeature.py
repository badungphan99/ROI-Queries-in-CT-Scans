...
class HaarlikeFeature:
    ...
    def _getIntegralImage(self, ognImage):
        ...
        for y in range(1, h + 1):
            for x in range(1, w + 1):
                itgImage[y, x] = itgImage[y, x - 1] + itgImage[y - 1, x] - itgImage[y - 1, x - 1] + ognImage[
                    y - 1, x - 1]
    ...
    def _getFeatureIn(self, itgImage, feature_type, x, y, w, h):
        ...
        if feature_type == HaarlikeType.TWO_HORIZONTAL:
            ...
            white = self._getSumIn(itgImage, x, y, w / 2, h / 2) + self._getSumIn(itgImage, x + w / 2, y + h / 2, w / 2,
                                                                                  h / 2)
            black = self._getSumIn(itgImage, x + w / 2, y, w / 2, h / 2) + self._getSumIn(itgImage, x, y + h / 2, w / 2,
                                                                                          h / 2)
